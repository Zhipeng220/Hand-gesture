...
This project aims to build a high-accuracy, low-latency sign language recognition system for barrier-free communication. The system innovatively combines anatomical constraints with spatial attention mechanisms to improve the accuracy and robustness of gesture recognition, and achieves real-time inference through optimized deployment on embedded platforms.
...
Place sign language video files in the `data/video/` directory:

```bash
mkdir -p data/video
# Copy video files to data/video/
cp /path/to/your/videos/*.mp4 data/video/
```
...
### Q1: What if MediaPipe can't detect hands?

**A:** 
1. Ensure hands occupy at least 30% of the frame
2. Improve image brightness and contrast
3. Adjust MediaPipe confidence threshold:
```python
hands = mp_hands.Hands(
    max_num_hands=2,
    min_detection_confidence=0.5,  # Lower detection threshold
    min_tracking_confidence=0.5
)
```

### Q2: CUDA Out of Memory error during training?

**A:**
1. Reduce batch_size: `batch_size = 8`
2. Lower model complexity: `embed_dim = 32`
...
### Q3: How to add new gesture classes?

**A:**
1. Add new gesture videos to `data/video/` following naming convention
2. Re-execute Steps 3-7 in complete workflow
3. Model will automatically recognize new classes and retrain

### Q4: Model accuracy drops in new environment?

**A:**
1. Collect data from new environment for fine-tuning
2. Increase data augmentation intensity
3. Use transfer learning:
```python
# Freeze early layers, train only classification layer
for param in model.coord_embed.parameters():
    param.requires_grad = False
```

### Q5: How to improve inference speed?

**A:**
1. Use TensorRT for model acceleration
2. Reduce input resolution
3. Use model pruning or quantization
4. Batch inference for multiple frames
...
@misc{handfind2024,
  author = {Your Name},
  title = {Sign Language Recognition System Based on Anatomical Constraints and Spatial Attention Mechanisms},
  year = {2024},
  publisher = {GitHub},
  url = {https://github.com/Scatteredpeople/HandFind}
}
```
